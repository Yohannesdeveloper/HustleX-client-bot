# Install HustleX Bot Service with Correct Path
...
if (-not (Test-Path $nssmPath)) {
    ...
    Write-Host "Please run setup_24x7.ps1 first to download NSSM" -ForegroundColor Yellow
    exit 1
}
...
# Service runner script path
$serviceRunner = Join-Path $ProjectDir "service_runner.py"

Write-Host "Python executable: $pythonExe" -ForegroundColor Cyan
Write-Host "Service runner: $serviceRunner" -ForegroundColor Cyan
...
& $nssmPath install $ServiceName $pythonExe $serviceRunner
